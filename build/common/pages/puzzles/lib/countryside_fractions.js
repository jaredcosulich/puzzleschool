...
var LASER_HEIGHT, OBJECTS, countrysideFractions, direction, direction2, directions, index, index2, perpendicularDirections, shuffle, _i, _j, _len, _len1,
  __slice = [].slice,
...
countrysideFractions = typeof exports !== "undefined" && exports !== null ? exports : provide('./lib/countryside_fractions', {});
...
countrysideFractions.ChunkHelper = (function() {
...
countrysideFractions.ViewHelper = (function() {

  ViewHelper.prototype.baseFolder = '/assets/images/puzzles/countryside_fractions/';
  ...
  ViewHelper.prototype.showFraction = function(squareOrLaser) {
    var beam, css, denominator, fraction, numerator, pos,
      _this = this;
    ...
    if (squareOrLaser.hasClass('laser')) {
      ...
      if (beam.width() > beam.height()) {
        ...
        if (beam.width() > this.board.find('.square').width()) {
          pos += (beam.width() / 2) - (fraction.width() / 2);
        }
        css[beam.hasClass('left') ? 'right' : 'left'] = pos;
      } else {
        css.left = (beam.width() / 2) - (fraction.width() / 2);
        pos = fraction.height() + 3;
        if (beam.height() > this.board.find('.square').height()) {
          pos += (beam.height() / 2) - (fraction.height() / 2);
        }
        css[beam.hasClass('up') ? 'bottom' : 'top'] = pos;
      }
      return fraction.css(css);
    } else {
  ...
  ViewHelper.prototype.initMovableObject = function(square, callback) {
    ...
    moveObject = function(e) {
      ...
      move = function(e) {
        ...
        left = _this.clientX(e) - (square.width() / 2);
        top = _this.clientY(e) - (square.height() / 2);
        movingObject.css({
          left: left,
          top: top
        });
        ...
        for (_k = 0, _len2 = _ref.length; _k < _len2; _k++) {
          ...
          if (_this.clientX(e) >= offset.left && _this.clientX(e) < offset.left + offset.width && _this.clientY(e) >= offset.top && _this.clientY(e) < offset.top + offset.height) {
