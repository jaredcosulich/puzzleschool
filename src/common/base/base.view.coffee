...
new class GameMatch extends soma.View
    selector: '#base'
    ...
    showRegistration: () ->
        ...
        submitForm = () =>
            ...
            $.ajaj
                url: '/api/register'
                method: 'POST'
                headers: { 'X-CSRF-Token': context.cookies.get('_csrf', {raw: true}) }
                ...
                success: () =>
                    ...
                    if window.postRegistration.length
                        ...
                        @checkLoggedIn()
                        if (go = context.cookies.get('returnTo'))
    ...
    checkLoggedIn: () ->
        return unless (@user = context.cookies.get('user'))?
